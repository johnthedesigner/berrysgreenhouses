...
class Cuztom
{
	...
	function register_styles()
	{
		wp_register_style( 'cuztom_css', 
			get_template_directory_uri() . '/css/cp_style.css', 
			false, 
			CUZTOM_VERSION, 
			'screen'
		);
	...
	function register_scripts()
	{
		wp_register_script( 'cuztom_js', 
			get_template_directory_uri() . '/js/cp_functions.js', 
...
class Cuztom_Meta_Box
{
	...
	function output_field( $field_id_name, $field, $meta, $number='', $i=0 )
	{
		switch( $field['type'] ) :
			...
			case 'text' :
				echo '<input type="text" name="cuztom[' . $field_id_name .']'.$number.'" class="custom_post_field" id="' . $field_id_name . $number . '" value="' . $meta[$i] . '" />';

			break;
			
			case 'textarea' :
				echo '<textarea name="cuztom[' . $field_id_name .']'.$number.'" class="custom_post_field" id="' . $field_id_name . $number . '">' . $meta[$i] . '</textarea>';
			break;
			
			case 'checkbox' :
				echo '<input type="checkbox" name="cuztom[' . $field_id_name .']'.$number.'" class="custom_post_field" id="' . $field_id_name . $number . '" ' . checked( $meta[$i], 'on', false ) . ' />';
			break;
			
			case 'yesno' :
				echo '<input type="radio" name="cuztom[' . $field_id_name .']'.$number.'" class="custom_post_field" id="' . $field_id_name . $number . '_yes" value="yes" ' . checked( $meta[0], 'yes', false ) . ' />';
				echo '<label for="' . $field_id_name . '_yes">' . __('Yes') . '</label>';
				
				echo '<input type="radio" name="cuztom[' . $field_id_name . $number .']" class="custom_post_field" id="' . $field_id_name . $number . '_no" value="no" ' . checked( $meta[0], 'no', false ) . ' />';
				echo '<label for="' . $field_id_name . '_no">' . __('No') . '</label>';
			break;
			
			case 'select' :
				echo '<select name="cuztom[' . $field_id_name .']'.$number.'" class="custom_post_field" id="' . $field_id_name . $number . '">';
			...
			case 'checkboxes' :
				foreach( $field['options'] as $slug => $name )
				{
					echo '<input type="checkbox" name="cuztom[' . $field_id_name . '][]" class="custom_post_field" id="' . $field_id_name . $number . '_' . Cuztom::uglify( $slug ) . '" value="' . Cuztom::uglify( $slug ) . '" ' . ( in_array( Cuztom::uglify( $slug ), maybe_unserialize( $meta[0] ) ) ? 'checked="checked"' : '' ) . ' /><label for="' . $field_id_name . '_' . Cuztom::uglify( $slug ) . '">' . Cuztom::beautify( $name ) . '</label>';
				}
			break;
			...
			case 'radio' :
				foreach( $field['options'] as $slug => $name )
				{
					echo '<input type="radio" name="cuztom[' . $field_id_name .']'.$number.'" class="custom_post_field" id="' . $field_id_name . $number . '_' . Cuztom::uglify( $slug ) . '" value="' . Cuztom::uglify( $slug ) . '" ' . checked( Cuztom::uglify( $slug ), $meta[0], false ) . ' /><label for="' . $field_id_name . '_' . Cuztom::uglify( $slug ) . '">' . Cuztom::beautify( $name ) . '</label>';
				}
			break;
			...
			case 'image' :
				$image = get_template_directory_uri().'/img/photo.png';	
				echo '<span class="custom_default_image" style="display:none">'.$image.'</span>';
				if ($meta[$i]) { $image = wp_get_attachment_image_src($meta[$i], 'thumbnail');	$image = $image[0]; }				
				echo	'<input name="cuztom[' . $field_id_name .']'.$number.'" type="hidden" class="custom_upload_image custom_post_field" id="' . $field_id_name . $number . '" value="'.$meta[$i].'" />
			...
			case 'date' :
				echo '<input type="text" name="cuztom[' . $field_id_name .']'.$number.'" id="' . $field_id_name . $number . '" class="cuztom_datepicker datepicker custom_post_field"  />';
			break;
				...
				if ($meta) {
					foreach($meta as $row) {
						echo '<li><span class="sort handle">|||</span>
									<input type="text" name="cuztom['.$field_id_name.']['.$i.']" class="custom_post_field" id="'.$field_id_name.$number.'" value="'.$row[0][0].'" size="30" />
									<a class="repeatable-remove button" href="#">-</a></li>';
						$i++;
					}
				} else {
					echo '<li><span class="sort handle">|||</span>
								<input type="text" name="cuztom['.$field_id_name.']['.$i.']" class="custom_post_field" id="'.$field_id_name.$number.'" value="" size="30" />
								<a class="repeatable-remove button" href="#">-</a></li>';
				}
				echo '</ul>';
			break;
			...
			case 'repeatwrap':
				...
				$first = '_' . Cuztom::uglify( $this->box_title ) . '_' . $field['name'] . '_' . Cuztom::uglify( $field_id['name'] );
				...
				if ($meta_first) {
					...
					for($i = 0; $i < $count; $i++){
						...
						echo '<li class="fieldset postbox"><h3 class="sort hndle">||| <em>Drag to reorder</em></h3>';
						...
						foreach( $field['contents'] as $subfield )
						{
							...
							global $post;
							$number = '['.$i.']';
							$field_id_name = '_' . Cuztom::uglify( $this->box_title ) . '_' . $parent . '_' . Cuztom::uglify( $subfield['name'] );
							$get_field=$field_id_name.$number;
							$meta = get_post_meta( $post->ID, $field_id_name, true );
											
							echo '<div class="subfield pad">';
							echo '<label for="' . $field_id_name . '" class="cuztom_label">' . $subfield['label'] . '</label>';
							echo '<div class="cuztom_description description">' . $subfield['description'] . '</div>';
					...
					echo '<ul id="'.$field_id_name.'-repeatable" class="custom_repeatable">
							<li class="fieldset postbox"><h3 class="sort hndle">||| <em>Drag to reorder</em></h3>';
					...
					foreach( $field['contents'] as $field )
					{
						...
						global $post;
						$number = '['.$i.']';
						$field_id_name = '_' . Cuztom::uglify( $this->box_title ) . '_' . $parent . '_' . Cuztom::uglify( $field['name'] );
						$get_field=$field_id_name.$number;
						$meta = get_post_meta( $post->ID, $field_id_name, true );
	...
	{		
		...
		if( ! empty( $this->meta_fields ) )
		{
			foreach( $this->meta_fields as $field )
			{
				...
				foreach ($_POST['cuztom'] as $key => $value){
